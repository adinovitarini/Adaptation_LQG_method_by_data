function dataset = GenerateSeq_VI(sys,N,Rww,Rvv,Q,R)
...
% [K,~,~] = lqr(sys,Q,R);
[~,K,~] = value_iteration(sys.A,N,sys.B,Q,R);
...
tic
for i = 1:N
    x(:,i+1) = (sys.A-sys.B*K(i))*x(:,i)+w(i);
    y(i) = (sys.C*x(:,i))+v(i);
    x(:,i) = x(:,i+1);
    x_hat(:,i+1) = ((sys.A-sys.B*K(i))*x_hat(:,i))+L(i)*(y(i)-y_hat(i));
    y_hat(i) = sys.C*x_hat(:,i);
    x_hat(:,i) = x_hat(:,i+1);
    x_nw(:,i+1) = (sys.A-sys.B*K(i))*x_nw(:,i);
    y_nw(i) = (sys.C*x_nw(:,i));
    x_nw(:,i) = x_nw(:,i+1);
end
time_elapsed = toc;
